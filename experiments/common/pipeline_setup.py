"""Shared pipeline setup: cached mesh loading, OG building, surface sampling."""
...
def _check_normals(normals_np: np.ndarray, threshold_deg: float = 30.0) -> bool:
    """Check if normals have sufficient angular diversity.

    Returns True if normals are valid (diverse), False if degenerate.
    """
    ...
    if norm < 1e-8:
        return True  # normals cancel out → they're diverse
    mean_normal /= norm
    dots = normals_np @ mean_normal
...
class PipelineContext:
    """Cached pipeline context for a single model.

    Expensive operations (mesh loading, OG building, surface sampling)
    are computed once and reused across experiments.
    """
    ...
    def load_mesh(self):
        """Load and transform mesh. Cached after first call."""
    ...
    def sample_surface(self, num_points: int | None = None, seed: int = 42):
        ...
        if not _check_normals(norms_np):
            raise DegenerateNormalsError(
                ...
                f"(all pointing in roughly the same direction)."
            )
    ...
    def build_sampling_og(self):
        """Build surface-only occupancy grid for viewpoint sampling. Cached."""
    ...
    def build_visibility_query(self, method: str = "raycast"):
        """Build visibility query object. Cached per method."""
    ...
    def build_sampler(self, strategy: str = "targeted"):
        """Build a viewpoint sampler. Cached per strategy."""
    ...
    def mesh_bounds(self):
        """Return (bounds_min, bounds_max) as numpy arrays."""
        tm, _ = self.load_mesh()
