...
def compute_all_lbs(
    ...
    *,
    ...
    if (vrp_objective_value_m is not None
            and out[_CUOPT_FIELD] > vrp_objective_value_m + 1e-6):
        logger.warning(
            "cuOpt best_bound (%.3f) > observed objective (%.3f); "
            "reading wrong cuOpt field?",
            out[_CUOPT_FIELD], vrp_objective_value_m,
        )
    # Info: analytical LB should be ≤ cuOpt bound when both present.
    if (out[_CUOPT_FIELD] > 0 and out["vrp_objective_lb_m"] > 0
            and out["vrp_objective_lb_m"] > out[_CUOPT_FIELD] + 1e-6):
        logger.info(
            "Analytical LB (%.3f) beats cuOpt bound (%.3f) — "
            "cuOpt likely terminated very early.",
