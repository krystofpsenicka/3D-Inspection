"""Result persistence: save/load experiment results as JSON + NPZ."""
...
def save_run_result(result: dict, path: str) -> None:
    """Save an experiment result dict as JSON + NPZ.

    Numpy/CuPy arrays go into {path}.npz, everything else into {path}.json.
    """
...
def load_run_result(path: str) -> dict:
    """Load a result saved by save_run_result."""
    result = {}
    json_path = path + ".json"
    npz_path = path + ".npz"
...
def aggregate_to_csv(results_dir: str, output_csv: str) -> None:
    """Read all per-run JSON files in a directory and flatten to CSV."""
    import csv
