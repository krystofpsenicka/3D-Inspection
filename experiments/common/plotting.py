...
def setup_thesis_style():
    ...
    # Patch set_title/suptitle centrally rather than editing ~30+ call sites.
...
def panel_title(ax, text: str, **kwargs):
    """Per-subplot title, bypassing the global suppression patch."""
...
def save_figure(fig, path: str, formats: list[str] | None = None):
    """Save in multiple formats (default: pdf + png)."""
    if formats is None:
        formats = ["pdf", "png"]
...
def grouped_bar(
    ...
    """data: {group: [values_per_x]}; yerr: {group: [std_per_x]}."""
...
def stacked_bar(
    ...
    """Stacked bar for timing breakdowns."""
...
def log_log_with_fit(ax, x, y, label: str = "", color=None):
    ...
    if len(x) >= 2:
        ...
        ax.loglog(x_fit, y_fit, "--", color=c, alpha=0.5, label=f"slope={coeffs[0]:.2f}")
...
def convergence_plot(
    ax, histories: dict[str, list[list[float]]], ylabel: str = "Coverage", title: str = ""
):
    """Convergence: mean + shaded std across seeds.
    histories: {method: [seed_1_history, seed_2_history, ...]}
    """
...
def cdf_plot(ax, data: dict[str, list[float]], xlabel: str = "", title: str = ""):
    """Empirical CDFs for multiple distributions."""
...
def heatmap_annotated(
    ...
    """Annotated heatmap. Text colour flips at viridis luminance midpoint
    (~0.55). overlay_mask draws diagonal hatching on True cells.
    """
...
def pareto_front(
    ...
    """Scatter with dominated points greyed out."""
    x, y = np.asarray(x, dtype=float), np.asarray(y, dtype=float)
    n = len(x)
    ...
    ax.scatter(x[dominated], y[dominated], c="lightgray", marker="o", s=30, zorder=1, label="Dominated")
    ax.scatter(
        x[~dominated], y[~dominated],
        c=CATEGORICAL_COLORS[0], marker="o", s=50, zorder=2, label="Pareto front",
    )
    ...
    if labels:
        for i in range(n):
            ax.annotate(labels[i], (x[i], y[i]), fontsize=6, textcoords="offset points", xytext=(3, 3))
...
def optimality_gap_bar(
    ax, solutions: dict[str, float], lower_bound: float,
    ylabel: str = "Optimality ratio", title: str = "",
):
    """Bar of solution / lower_bound ratio per method."""
...
def radar_chart(fig, metrics: dict[str, dict[str, float]], title: str = ""):
    """Radar/spider chart. metrics: {method: {metric: normalised 0..1}}."""
