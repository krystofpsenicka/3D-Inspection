...
def heatmap_annotated(
    ...
    for i in range(len(row_labels)):
        for j in range(len(col_labels)):
            ...
            ax.text(
                j, i, f"{v:{fmt}}", ha="center", va="center", fontsize=7, color=text_color, zorder=5
            )
...
def pareto_front(
    ax, x, y, labels: list[str] | None = None, xlabel: str = "", ylabel: str = "", title: str = ""
):
    ...
    ax.scatter(
        x[dominated], y[dominated], c="lightgray", marker="o", s=30, zorder=1, label="Dominated"
    )
    ax.scatter(
        x[~dominated],
        y[~dominated],
        c=CATEGORICAL_COLORS[0],
        marker="o",
        s=50,
        zorder=2,
        label="Pareto front",
    ...
    if labels:
        for i in range(n):
            ax.annotate(
                labels[i], (x[i], y[i]), fontsize=6, textcoords="offset points", xytext=(3, 3)
            )
