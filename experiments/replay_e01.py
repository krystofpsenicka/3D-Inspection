#!/usr/bin/env python3
"""Replay E01: Sampling Strategy Comparison in Isaac Sim.

Loads saved viz data for one sampling strategy run and builds an interactive
Isaac Sim stage showing the mesh, selected viewpoints (post set-cover), and
uncovered surface points.

Usage:
    conda run -n isaaclab python experiments/replay_e01.py --strategy targeted_50
    conda run -n isaaclab python experiments/replay_e01.py --list
"""
...
VIZ_DIR = os.path.join(RESULTS_DIR, "e01_sampling_strategy", "viz")


def _list_available():
    if not os.path.isdir(VIZ_DIR):
        print(f"No viz data found at {VIZ_DIR}")
        print("Run e01 first: conda run -n isaaclab python -m experiments.e01_sampling_strategy"
              " --section A --seeds 42")
        return
...
def _load_viz(strategy: str, seed: int = 42):
    name = f"strategy={strategy}_seed={seed}"
...
def main():
    p = argparse.ArgumentParser(description="Replay E01: Sampling Strategy in Isaac Sim")
    p.add_argument("--strategy", default="targeted_50",
                   help="Sampling strategy to replay (e.g. targeted_50, weighted, cmaes_50)")
    ...
    # ── Load viz data ──────────────────────────────────────────────────────
    logger.info("Loading viz data for strategy=%s seed=%d", args.strategy, args.seed)
    data = _load_viz(args.strategy, args.seed)
    ...
    # ── Reconstruct OptimizationResult-like object ─────────────────────────
    import cupy as cp
    ...
    viz.add_solution(stage, "/World/E01", result)

    logger.info("Scene ready — %d viewpoints for strategy=%s (coverage=%.2f%%)",
                result.num_viewpoints, args.strategy, result.total_coverage * 100)
