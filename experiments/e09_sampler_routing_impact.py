...
try:
    ...
    from experiments.common.runner import (
        set_seed, timed, free_gpu_memory, handle_row_exception,
    )
...
def _cmaes_travel_weights(model_name: str) -> list[float]:
    ...
    return (list(E03_C_TRAVEL_WEIGHTS_DUKE) if _is_duke(model_name)
...
def _generate_plots_for_model(ok: list[dict], fig_dir: str, model_name: str,
    ...
    method_color = {cfg: CATEGORICAL_COLORS[i % len(CATEGORICAL_COLORS)]
    ...
    def _scatter(metric_y: str, ylabel: str, title: str, fname: str):
        ...
        for cfg in run_cfgs:
            ...
            if ok_rows:
                ax.scatter([r["num_viewpoints"] for r in ok_rows],
                           [r[metric_y] for r in ok_rows],
                           color=color, marker="o", s=40, alpha=0.85,
                           label=label)
            if bad_rows:
                ax.scatter([r["num_viewpoints"] for r in bad_rows],
                           [r[metric_y] for r in bad_rows],
                           color=color, marker="*", s=110, alpha=0.85,
        ...
        if any(r["coverage"] < target_coverage for r in ok):
            ...
            handles.append(Line2D([0], [0], marker="*", color="white",
                                  markeredgecolor="0.25", markerfacecolor="0.6",
                                  markersize=10,
                                  label=f"Coverage < {target_coverage*100:.0f}%"))
            labels.append(f"Coverage < {target_coverage*100:.0f}%")
            ax.legend(handles=handles, labels=labels, fontsize=6, ncol=2,
                      loc="best")
        else:
...
def main():
    ...
    p.add_argument("--models", nargs="+", default=_E09_MODELS,
    ...
    p.add_argument("--output_dir",
                   default=os.path.join(RESULTS_DIR, "e09_sampler_routing_impact"))
