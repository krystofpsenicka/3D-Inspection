...
class HPRO(nn.Module):
    ...
    def __init__(self, fits_in_memory=True, visiblity_score_thresh=0.99, device=None):
    ...
    def detect_max_in_direction(self, batch_size, centered_points, directions, gamma, n_pts, use_linear_kernel,
        ...
        if self.fits_in_memory:
            ...
            # transformed_points_with_noise_norm is (B, 1, N); squeeze the singleton
            # channel dim so the subtraction is (B, N) - (B, N). Without this the op
            # broadcasts to (B, B, N) and is only correct for batch_size == 1.
            w = (transformed_points_with_noise_norm.squeeze(1) - mk_k[0][:, :, k - 1]) / (
                    mk_k[0][:, :, 0] - mk_k[0][:, :, k - 1])
            w = self.elu(w)
        else:
            ...
            for i in range(n_pts):
                ...
                # [:, 0, i] -> (B,) so the per-point assignment is (B,) - (B,);
                # [:, :, i] would be (B, 1) and broadcast to (B, B).
                w[:, i] = (transformed_points_with_noise_norm[:, 0, i] - mk_k[0][:, k - 1]) / (
                            mk_k[0][:, 0] - mk_k[0][:, k - 1])
            w = self.elu(w)
