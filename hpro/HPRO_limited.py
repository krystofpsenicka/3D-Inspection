...
class HPRO_limited(nn.Module):
    ...
    def compute_frustum_mask(
        ...
        h_tan = h_coord / (depth + eps)   # (B, N)
        v_tan = v_coord / (depth + eps)   # (B, N)
    ...
    def forward(
        ...
        visible_pts = pts[:, :, w1.squeeze()].to(self.device)
        visible_indices = torch.nonzero(w1.squeeze()).squeeze().to(self.device)
