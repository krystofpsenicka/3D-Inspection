...
class SamplingVisualizer:
    ...
    def _create_free_space_geometries(
        self,
        outside_positions,
        outside_weights,
        inside_positions,
    ...
    def visualize_resampling_progression(
        self,
        normal_vis_map,
        normal_candidates,
        targeted_vis_map,
        ...
        logger.info(
            "Phase 1: %d normal VPs, coverage=%.1f%%. Press Q to continue.",
            n_normal,
            normal_coverage_pct,
        )
        show_geometries(
        ...
        for t_idx in range(n_targeted):
            ...
            geometries += create_viewpoint_geometry(
                cur_pos, cur_orient, self.frustum_params, ORANGE, sphere_radius=0.2
            )
            ...
            logger.info(
                "Targeted VP %d/%d: +%d pts, coverage=%.1f%%. Press Q to continue.",
                t_idx + 1,
                n_targeted,
                len(newly_covered),
                cur_coverage_pct,
        ...
        for i, candidate in enumerate(all_candidates):
            ...
            geometries += create_viewpoint_geometry(pos, rotation, self.frustum_params, base_color)
        ...
        logger.info(
            "Final: %d normal + %d targeted VPs, coverage=%.1f%%. Press Q to close.",
            n_normal,
            n_targeted,
            final_coverage_pct,
        )
