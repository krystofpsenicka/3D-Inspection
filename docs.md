...
| `visualization_isaac/` | Isaac Sim wrapper + per-phase visualizers + replay |
...
- **`compute_distance_matrix(og, waypoints_xyz)`** (`core/distance_matrix.py:186`) — Builds the `(N, N)` float32 collision-free distance matrix in meters via cuGraph SSSP on the 26-connected occupancy graph. Runs in a subprocess to isolate RMM allocations, with optional `.npy` cache.
...
- **`solve_vrp(dist_matrix, num_vehicles, depots, alpha=1.0, backend=HIGHS, time_limit=120, mip_gap=0.05) -> VRPResult`** (`vrp/vrp_solver.py:32`) — Single entry point for VRP. Generates a nearest-neighbor warm-start, dispatches to the chosen backend, and returns a `VRPResult`. `alpha ∈ [0, 1]` blends makespan (1) and total distance (0).
...
- **`save_solution` / `load_solution`** (`utils/serialization.py:19`, `:47`) — Serialize an `ExecutionResult` as ragged NPZ (positions / velocities + offset arrays) plus a JSON sidecar (waypoints, fail counts).
...
| `OFFSETS_26 / WEIGHTS_26 / OFFSETS_27 / WEIGHTS_27` | CuPy | Neighbor offsets + Euclidean weights. |
...
- **`OptimizingSampler`** (`sampling/samplers/optimizing.py:57`) — Iterative resampler that drives an `OptimizationBackend` to search 6-DoF poses (`x, y, z, θ, φ, roll`), maximizing deficit-weighted coverage minus a travel-distance penalty.
- **`OptimizationBackend`** (`sampling/samplers/optimizing.py:28`) — Abstract black-box optimizer interface: `optimize(objective_fn, n_dims, popsize, maxiter, ...) -> cp.ndarray` minimizing a CuPy objective in `[0, 1]^D`.
...
All optimizers expose `optimize(target_coverage=0.95, max_viewpoints=100) -> OptimizationResult`.
...
- **`ExpansionIterativeSetCover`** (`set_cover/expansion.py`) — Wraps an inner optimizer with a local refinement step: every selection is locally re-sampled (probabilistic or CMA-ES) and only kept if it strictly improves uncovered coverage.
...
- **`build_occupancy_grid(mesh, padding, inflation_voxels, resolution, fill_interior, complement_fill, extra_free_points, extra_margin_voxels)`** (`grid_builder_utils.py:162`) — End-to-end builder: derives bounds, voxelizes the mesh, optionally fills the interior or inverts (`complement_fill` for inside inspection), inflates by `inflation_voxels`, and returns a planning-ready `OccupancyGrid`.
...
- **`voxelize_mesh(mesh, resolution)`** (`grid_builder_utils.py:71`) — Surface-only voxelization on GPU; returns `(raw_grid, filled_grid)` CuPy bool arrays (filled only when `fill_interior=True`).
...
| Visibility visualizers | `visibility/` | `ModelVisualizer`, `SamplingVisualizer`, `VisibilityVisualizer`, `SetCoverVisualizer`, `EsdfVisualizer`, `add_frustum_lineset`, `add_viewpoint_geometry`. |
| VRP visualizers | `vrp/` | `VRPVisualizer`, `ReplayVisualizer` (`convert_trajectories`, `traj8_to_pose`), `add_brov_robots`, `ROBOT_COLORS`. |
...
| `--seed` | `42` | |
| `--curvature_weighting` | on | Bias toward complex regions. |
...
| Script | Topic |
|---|---|
| `e01_sampling_strategy.py` | Three sampling strategies. |
...
| `e10_cross_model.py` | Generalization across mesh complexity (TOSCA). |
