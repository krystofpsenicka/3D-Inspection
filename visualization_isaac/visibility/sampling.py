...
class SamplingVisualizer:
    ...
    def add_free_space(self, stage, base_path: str,
        ...
        for tag, positions, weights, cmap_name in [
            ...
            paths.append(create_points_prim(
                stage, f"{base_path}/{tag}",
                positions, colors=colors, point_size=point_size))
    ...
    def add_resampling_phase1(self, stage, base_path: str,
                              normal_vis_map, normal_candidates,
                              point_size: float = 4.0) -> list[str]:
        ...
        if uncovered_indices:
            paths.append(create_points_prim(
                ...
                colors=(1.0, 0.0, 0.0), point_size=point_size))

        coverage_pct = len(normal_covered) / self.num_points * 100
        logger.info("Phase 1: %d normal VPs, coverage=%.1f%%",
                    n_normal, coverage_pct)
        ...
        **before** this step.  The caller is responsible for updating it
        ...
        for i in range(len(normal_candidates)):
            ...
            paths.append(create_sphere_prim(
                stage, f"{base_path}/normal_marker_{i}",
                position=pos, radius=0.2, color=BLUE))
        ...
        for j in range(step_idx):
            ...
            paths.append(create_sphere_prim(
                stage, f"{base_path}/targeted_marker_{j}",
                position=pos, radius=0.2, color=ORANGE))
        ...
        if cumulative_covered:
            paths.append(create_points_prim(
                stage, f"{base_path}/already_covered",
                self.target_points[list(cumulative_covered)],
                colors=GRAY, point_size=point_size))

        if newly_covered:
            paths.append(create_points_prim(
                stage, f"{base_path}/newly_covered",
                self.target_points[list(newly_covered)],
                colors=GREEN, point_size=point_size))

        if still_uncovered:
            paths.append(create_points_prim(
                stage, f"{base_path}/still_uncovered",
                self.target_points[list(still_uncovered)],
                colors=RED, point_size=point_size))

        cur_coverage_pct = (len(cumulative_covered) + len(newly_covered)) / self.num_points * 100
        logger.info("Targeted VP %d/%d: +%d pts, coverage=%.1f%%",
                    step_idx + 1, len(targeted_candidates),
                    len(newly_covered), cur_coverage_pct)
    ...
    def add_resampling_phase3(self, stage, base_path: str,
                              normal_vis_map, normal_candidates,
        ...
        for i, candidate in enumerate(all_candidates):
            ...
            if len(visible_indices) > 0:
                paths.append(create_points_prim(
                    ...
                    self.target_points[visible_indices],
                    colors=vis_color, point_size=point_size))

        final_coverage_pct = len(total_covered) / self.num_points * 100
        logger.info("Final: %d normal + %d targeted VPs, coverage=%.1f%%",
                    n_normal, len(targeted_candidates), final_coverage_pct)
